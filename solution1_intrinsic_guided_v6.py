...
from differentiable_checker_v2 import differentiable_constraint_loss_v2
...
# Load Adnan's ThetaBranch (differentiable θ reader)
m = load("factored_clean_noaug_best", device)
branch = m['branch']
branch.eval()
print(f"✅ Adnan ThetaBranch loaded!")
...
test_imgs_96 = test_data['frame']

# Calibration constants for R(a_i)
CALIB_X_SLOPE = 0.01329
CALIB_X_INTERCEPT = -0.9906
CALIB_Y_SLOPE = -0.02001
CALIB_Y_INTERCEPT = 1.3958

def reverse_diffusion_guided_v2(score_net, z_unint_noisy_norm, z_fixed,
                                  diffusion, vae, z_unint_mean_t, z_unint_std_t,
                                  x_orig, y_orig, theta_orig, branch,
                                  start_t=20, num_steps=100, guidance_scale=1.0):
    """
    Guided reverse diffusion with x, y, AND theta constraint guidance.
    """
    score_net.eval()
    ...
    for t_val in timesteps:
        ...
        # Step 2 — Constraint guidance (x, y, AND theta)
        ...
        # Differentiable constraint loss v2 — includes theta
        constraint_loss, x_err, y_err, theta_err = differentiable_constraint_loss_v2(
            img_decoded, x_orig, y_orig, theta_orig, branch,
            eps_x=0.02, eps_y=0.01, eps_theta=0.30
        )

        # Step 3 — Gradient guidance
...
print(f"\nRunning Solution #1 guided v6 (x,y,θ guidance) on {len(visible_indices)} frames...")
print(f"guidance_scale={guidance_scale}, eps_x=0.02, eps_y=0.01, eps_theta=0.30")

for idx in tqdm(visible_indices):
    true_x, true_y, true_theta = test_states[idx,0], test_states[idx,1], test_states[idx,2]

    # Compute R(a_i) from original image — label free
    img_orig = test_imgs[idx].astype(np.float32) / 255.0
    img_orig_tensor = torch.FloatTensor(img_orig).permute(2,0,1).unsqueeze(0).to(device)

    from differentiable_checker_v2 import soft_purple_mask, soft_centroid
    with torch.no_grad():
        mask_orig = soft_purple_mask(img_orig_tensor)
        cx_orig, cy_orig = soft_centroid(mask_orig)
        x_orig_t = (CALIB_X_SLOPE * cx_orig + CALIB_X_INTERCEPT)
        y_orig_t = (CALIB_Y_SLOPE * cy_orig + CALIB_Y_INTERCEPT)
        theta_orig_t = torch.tensor([true_theta], dtype=torch.float32, device=device)
    ...
    for iteration in range(max_iterations + 1):
        ...
        # Guided reverse diffusion with x, y, theta guidance
        ...
        l_corr_norm = reverse_diffusion_guided_v2(
            score_net, l_noisy_norm, z_fixed,
            diffusion, vae, z_unint_mean_t, z_unint_std_t,
            x_orig_t, y_orig_t, theta_orig_t, branch,
...
np.save('/blue/iruchkin/aj.byreddy/tea_lab/results_solution1_intrinsic_guided_v6.npy', results)
print("✅ Results saved!")

print("\n" + "="*65)
print("SOLUTION #1 GUIDED v6 — x,y,θ guidance, g=1.0")
print("="*65)
